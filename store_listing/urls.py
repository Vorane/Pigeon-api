...
from product_listing.urls import outlet_product_urls, outlet_inventory_urls
from categories.urls import outlet_subcategories_urls

from .api import StoresView, OutletsView, StoreOutletsView, StoreTypesView
...
store_listing_urls = [
    # store specific urls
    url(r"^storetypes/$", StoreTypesView.as_view(), name="store-types-view"),
    url(r"^store/$", StoresView.as_view(), name="store"),
    url(
        r"^store/(?P<id>\d+)/outlets/$",
    ...
    url(
        r"^store/(?P<store_id>\d+)/outlet/(?P<outlet_id>\d+)/collections/",
    ...
    url(
        r"^outlet/(?P<outlet_id>\d+)/subcategories/", include(outlet_subcategories_urls)
    ),
    url(r"^outlet/(?P<outlet_id>\d+)/products/", include(outlet_product_urls)),
    url(r"^outlet/(?P<outlet_id>\d+)/inventory/", include(outlet_inventory_urls)),
]
